 ...
 */
public class ValidacaoTributacao implements Processo<PropostaAdesaoDTO> {
	...
	public void executar(PropostaAdesaoDTO propostaAdesaoDTO) throws ExecucaoProcessoException {
		...
		// Obtém o validador
		ValidationResult resultadoValidacao = new TributacaoValidator().validate(propostaAdesaoDTO);
	...
	public static Predicate<PropostaAdesaoDTO> deveExecutar() {
		return new Predicate<PropostaAdesaoDTO>() {
			...
			public boolean apply(PropostaAdesaoDTO propostaAdesao) {
				return !propostaAdesao.getTipoValidacao().equals(TipoValidacaoEnum.TRIBUTACAO);
			}
		};
	}
